...
- Coordinates 5-agent teams: Team Lead, Backend Builder, Frontend Builder, Test Engineer, Test Reviewer
...
/plugin marketplace add nipsmg/teamwerk
/plugin install teamwerk@nipsmg
...
Teamwerk includes 6 skills that are automatically loaded by agents when they join the team:
...
| **test-engineer** | Writes and executes all tests (API and E2E), captures screenshot evidence, and generates the HTML evidence report that serves as the primary deliverable |
...
4. **Test Engineer** writes tests after features are implemented. Submits tests to the Test Reviewer before generating the final report.
...
6. The cycle repeats until all tests pass review. The Test Engineer then generates the HTML evidence report with AC traceability matrix, screenshots, API response logs, and coverage gap analysis.
